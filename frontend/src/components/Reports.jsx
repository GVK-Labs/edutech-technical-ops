...
export default function Reports() {
  ...
  const [inventoryConsumption, setInventoryConsumption] = useState({
    ramUsage: [],
    storageUsage: [],
  });
  const [inventoryConsumptionLoading, setInventoryConsumptionLoading] =
    useState(false);
  ...
  useEffect(() => {
    if (activeTab === "assemblies" && assemblies.length === 0)
      fetchAssemblies();
    if (activeTab === "technician" && techPerformance.length === 0)
      fetchTechPerformance();
    if (activeTab === "inventory" && inventoryConsumption.ramUsage.length === 0)
    ...
    inventoryConsumption.ramUsage.length,
    jobProgress.length,
  ]);
  ...
  const fetchAssemblies = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/reports/assembly-details`, {
  ...
  const fetchUnitHistory = async (e) => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/reports/unit-history?serial=${searchSerial}`,
        { withCredentials: true },
      );
      if (res.data.Status) {
        if (!res.data.data) {
  ...
  const fetchTechPerformance = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/reports/technician-performance`, {
        withCredentials: true,
      });
      ...
      setTechPerformanceLoading(false);
    }
  };
  ...
  const fetchInventoryConsumption = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/reports/inventory-consumption`, {
        withCredentials: true,
      });
      ...
      setInventoryConsumptionLoading(false);
    }
  };
  ...
  const fetchJobProgress = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/reports/job-progress`, {
        withCredentials: true,
      });
  ...
  const handleExportExcel = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/reports/assembly-details/export/excel`,
  ...
  const handleExportPDF = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/reports/assembly-details/export/pdf`,
        { withCredentials: true, responseType: "blob" },
      );
  ...
  const handleExportInventoryExcel = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/reports/inventory/export/excel`, {
        withCredentials: true,
        responseType: "blob",
      });
      ...
      toast.success("Inventory workbook exported successfully");
    } catch {
      toast.error("Failed to export inventory workbook");
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Explore unit histories, performance metrics, and inventory usage.
          </p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-2 md:grid-cols-5 h-auto md:h-10 mb-6">
          <TabsTrigger value="assemblies" className="gap-2">
            <BarChart2 className="h-4 w-4" /> All Assemblies
          </TabsTrigger>
          <TabsTrigger value="history" className="gap-2">
            <History className="h-4 w-4" /> Unit History
          </TabsTrigger>
          <TabsTrigger value="technician" className="gap-2">
            <Users className="h-4 w-4" /> Tech Performance
          </TabsTrigger>
          <TabsTrigger value="inventory" className="gap-2">
            <Activity className="h-4 w-4" /> Inventory Usage
          </TabsTrigger>
          <TabsTrigger value="jobs" className="gap-2">
            <Target className="h-4 w-4" /> Job Progress
          </TabsTrigger>
        </TabsList>
            ...
            <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle className="text-lg">
                  Comprehensive Assembly Log
                </CardTitle>
                <CardDescription>
                  High-level overview. Export to Excel or PDF for full
                  hardware/software serials.
                </CardDescription>
              </div>
        ...
        <TabsContent value="history">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Search by OPS serial, Motherboard serial, or Job number to see a
                unit's complete lifecycle.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form
                onSubmit={fetchUnitHistory}
                className="flex gap-2 max-w-lg mb-8"
              >
                <Input
                  placeholder="Enter serial number or job number..."
                  value={searchSerial}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-mono">
                        {unitHistory.motherboard_serial}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-muted-foreground">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm">
                        {unitHistory.assembly_completed_at
                          ? new Date(
                              unitHistory.assembly_completed_at,
                            ).toLocaleString()
                          : "N/A"}
                        ...
                        {unitHistory.repairs.map((repair, idx) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-xs uppercase font-medium bg-amber-100 text-amber-800 px-2 py-0.5 rounded-full">
                                {repair.status}
                              </span>
                            </div>
        ...
        <TabsContent value="technician">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Number of successful assemblies completed by each technician.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="inventory">
          <div className="mb-4 flex flex-col gap-3 rounded-lg border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-semibold">Inventory workbook</p>
              <p className="text-sm text-muted-foreground">
                Export summary, stock status, specifications, serials, and
                batches into separate Excel sheets.
              </p>
            </div>
            <Button
            ...
            >
              ...
              {inventoryExporting ? "Exporting..." : "Export Inventory Excel"}
            </Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Total RAM modules used in assemblies.
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          cy="50%"
                          outerRadius={100}
                          label={({ capacity_gb, used_count }) =>
                            `${capacity_gb}GB (${used_count})`
                          }
                        >
                          {inventoryConsumption.ramUsage?.map(
                            (entry, index) => (
                              <Cell
                                key={`cell-${index}`}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Total storage devices used in assemblies.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {inventoryConsumptionLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          cy="50%"
                          outerRadius={100}
                          label={({ capacity_gb, storage_type, used_count }) =>
                            `${capacity_gb}GB ${storage_type} (${used_count})`
                          }
                        >
                          {inventoryConsumption.storageUsage?.map(
                            (entry, index) => (
                              <Cell
                                key={`cell-${index}`}
        ...
        <TabsContent value="jobs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Active job assembly completion status.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {jobProgressLoading ? (
                      ...
                      <div key={job.id} className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {job.completed_units} / {job.total_units} Assembled
                            ({percent}%)
                          </span>
                        </div>
