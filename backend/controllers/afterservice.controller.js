...
export const getRepairs = async (req, res) => {
  ...
  if (search) {
    where.push(
      "(rj.repair_number LIKE ? OR c.name LIKE ? OR io.serial_number LIKE ?)",
    );
...
export const getRepairDetail = async (req, res) => {
  try {
    ...
    const [storages] = await pool.query(
      ...
      WHERE ast.assembled_unit_id = ?`,
      [rj.unit_id],
    );

    const [[wifi]] = await pool
      .query(
        `
    ...
    const [software] = await pool.query(
      ...
      WHERE ams.assembled_unit_id = ? AND ams.is_active = 1`,
      [rj.unit_id],
    );
    ...
    const [replacements] = await pool.query(
      ...
      ORDER BY rcr.replacement_date DESC`,
      [req.params.id],
    );
...
export const createRepair = async (req, res) => {
  ...
  if (!assembled_unit_id || !client_id || !reported_issue?.trim())
    return res
      ...
      .json({
        Status: false,
        Error: "assembled_unit_id, client_id and reported_issue are required",
      });
  ...
  try {
    ...
    if (Number(unit.client_id) !== Number(client_id))
      throw new Error("Unit does not belong to this client");
    if (
      unit.job_type === "smartboard" ||
      unit.status !== "completed" ||
      unit.unit_status !== "delivered"
    )
    ...
    await conn.query(
      "UPDATE assembled_units SET status='in_repair' WHERE id=?",
      [assembled_unit_id],
    );
    await conn.commit();
    void logAction({
      ...
      details: { repair_number: repairNumber, assembled_unit_id },
      req,
    });
    res.json({
      Status: true,
...
export const updateStatus = async (req, res) => {
  ...
  try {
    ...
    const [[rj]] = await conn.query(
      "SELECT assembled_unit_id, status FROM repair_jobs WHERE id=?",
      [req.params.id],
    );
    if (!rj) throw new Error("Repair not found");

    const updates = { status };
    if (status === "in_progress" && rj.status === "open")
    ...
    const setCols = Object.keys(updates)
      .map((k) => `${k}=?`)
      .join(", ");
    await conn.query(`UPDATE repair_jobs SET ${setCols} WHERE id=?`, [
    ...
    if (status === "completed" || status === "closed")
      await conn.query(
        "UPDATE assembled_units SET status='delivered' WHERE id=?",
        [rj.assembled_unit_id],
      );
...
export const assignTechnician = async (req, res) => {
  ...
  try {
    await pool.query("UPDATE repair_jobs SET technician_id=? WHERE id=?", [
      technician_id || null,
      req.params.id,
    ]);
    res.json({ Status: true });
...
export const logReplacement = async (req, res) => {
  const { component_type, old_inventory_id, new_inventory_id, notes } =
    req.body;
  const valid = [
    "ops",
    "ram",
  ...
  try {
    ...
    if (component_type === "software_key") {
      ...
      if (!software_catalog_id || !license_key?.trim())
        throw new Error(
          "software_catalog_id and license_key required for software replacement",
        );

      // deactivate old
      const [[rj]] = await conn.query(
        "SELECT assembled_unit_id FROM repair_jobs WHERE id=?",
        [req.params.id],
      );
      await conn.query(
      ...
      if (new_inventory_id) {
        const tableMap = {
          ...
          wifi_card: "inventory_network_cards",
          flat_panel: "inventory_flat_panels",
        };
        const [[item]] = await conn.query(
          `SELECT status FROM ${tableMap[component_type]} WHERE id=?`,
          [new_inventory_id],
        );
        if (!item) throw new Error("New component not found");
        if (item.status !== "in_stock")
          throw new Error("New component is not in stock");
      }
      ...
      if (component_type === "ops" && new_inventory_id) {
        ...
        await conn.query(
          "UPDATE assembled_units SET ops_inventory_id=? WHERE id=?",
          [new_inventory_id, rj.assembled_unit_id],
        );
      }
      ...
      if (component_type === "wifi_card") {
        ...
        await conn.query(
          "UPDATE assembled_units SET wifi_card_inventory_id=? WHERE id=?",
          [new_inventory_id || null, rj.assembled_unit_id],
        );
      }
    ...
    void logAction({
      userId: req.user.id,
      action: "repair.component_replaced",
      entityType: "repair_job",
      entityId: Number(req.params.id),
      details: {
        component_type,
        old_inventory_id: old_inventory_id || null,
...
export const updateNotes = async (req, res) => {
  try {
    await pool.query("UPDATE repair_jobs SET notes=? WHERE id=?", [
      req.body.notes || null,
      req.params.id,
    ]);
    res.json({ Status: true });
