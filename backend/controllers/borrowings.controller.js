  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        COALESCE(i_o.serial_number, i_r.serial_number, i_s.serial_number, i_n.serial_number, i_f.serial_number) AS serial_number,
        CASE
          ...
          WHEN b.component_type = 'flat_panel' THEN s_f.model_name
        END AS item_details
      ...
      LEFT JOIN inventory_flat_panels i_f ON b.component_type = 'flat_panel' AND b.inventory_id = i_f.id
      LEFT JOIN flat_panel_models s_f ON i_f.model_id = s_f.id
      ${whereClause}
...
export const lendItem = async (req, res) => {
  ...
  if (!t)
    return res
      .status(400)
      .json({ Status: false, Error: "Invalid component_type" });
  ...
  try {
    ...
    const [[item]] = await conn.query(
      `SELECT id, status, serial_number FROM ${t.inv} WHERE id = ? FOR UPDATE`,
      [inventory_id],
    );
    ...
    if (item.status !== "in_stock") {
      throw new Error(
        `Cannot lend item because its status is '${item.status}'`,
      );
    }
    ...
    await conn.query(`UPDATE ${t.inv} SET status = 'borrowed' WHERE id = ?`, [
      inventory_id,
    ]);
...
export const scanInventory = async (req, res) => {
  ...
  if (!serial_number)
    return res
      .status(400)
      .json({ Status: false, Error: "serial_number is required" });

  try {
    const qOps =
      "SELECT id as inventory_id, 'ops' AS component_type, serial_number, status, (SELECT model_name FROM ops_models WHERE id=ops_model_id) as item_details FROM inventory_ops WHERE serial_number=?";
    const qRam =
      "SELECT id as inventory_id, 'ram' AS component_type, serial_number, status, (SELECT CONCAT(ddr_version, ' ', capacity_gb, 'GB') FROM ram_specs WHERE id=ram_spec_id) as item_details FROM inventory_rams WHERE serial_number=?";
    const qStorage =
      "SELECT id as inventory_id, 'storage' AS component_type, serial_number, status, (SELECT CONCAT(storage_type, ' ', form_factor, ' ', interface, ' ', capacity_gb, 'GB') FROM storage_specs WHERE id=storage_spec_id) as item_details FROM inventory_storage WHERE serial_number=?";
    const qNetwork =
      "SELECT id as inventory_id, 'network_card' AS component_type, serial_number, status, (SELECT model_name FROM network_card_models WHERE id=model_id) as item_details FROM inventory_network_cards WHERE serial_number=?";
    const qFlatPanel =
      "SELECT id as inventory_id, 'flat_panel' AS component_type, serial_number, status, (SELECT model_name FROM flat_panel_models WHERE id=model_id) as item_details FROM inventory_flat_panels WHERE serial_number=?";

    const queries = [qOps, qRam, qStorage, qNetwork, qFlatPanel];
...
export const lendBatch = async (req, res) => {
  ...
  if (!technician_id || !Array.isArray(items) || items.length === 0) {
    return res
      .status(400)
      .json({ Status: false, Error: "Missing technician_id or items" });
  }
  ...
  try {
    ...
    for (const item of items) {
      ...
      const [[invItem]] = await conn.query(
        `SELECT status, serial_number FROM ${t.inv} WHERE id=? FOR UPDATE`,
        [inventory_id],
      );
      if (!invItem || invItem.status !== "in_stock") {
        throw new Error(
          `Item ${invItem?.serial_number || inventory_id} is not in stock`,
        );
      }
...
export const returnItem = async (req, res) => {
  ...
  try {
    ...
    const [[borrowing]] = await conn.query(
      `SELECT * FROM technician_borrowings WHERE id = ? FOR UPDATE`,
      [borrowingId],
    );
    ...
    if (borrowing.status !== "borrowed")
      throw new Error(
        `Cannot return item because status is '${borrowing.status}'`,
      );
    ...
    await conn.query(
      `UPDATE technician_borrowings SET status = 'returned', returned_at = CURRENT_TIMESTAMP WHERE id = ?`,
      [borrowingId],
    );

    // Revert inventory status back to in_stock
    await conn.query(`UPDATE ${t.inv} SET status = 'in_stock' WHERE id = ?`, [
...
export const consumeItem = async (req, res) => {
  ...
  try {
    ...
    if (borrowing.status !== "borrowed")
      throw new Error(
        `Cannot consume item because status is '${borrowing.status}'`,
      );
    ...
    // Update borrowing status
    await conn.query(
      `UPDATE technician_borrowings SET status = 'consumed' WHERE id = ?`,
      [borrowingId],
    );

    // Mark inventory as assigned (since it is consumed in the field)
    await conn.query(`UPDATE ${t.inv} SET status = 'assigned' WHERE id = ?`, [
      borrowing.inventory_id,
    ]);
